...
\date{March 24, 2026}  % if the \date{} command is left out, the current date will be used
\begin{document}
...
Here is the LaTeX code that derives the forward and backward propagation equations for your specific MLP architecture, including bias terms, using both coordinate and matrix representations.


\section{Notation and Architecture}
We consider a Multi-Layer Perceptron (MLP) with:
\begin{itemize}
    \item Input layer: 2 neurons ($x_1, x_2$) plus a bias unit ($x_0 = 1$).
    \item First hidden layer: 4 neurons ($z^{(1)}_1 \dots z^{(1)}_4$) plus a bias unit ($z^{(1)}_0 = 1$). The weighted sum before activation is $h^{(1)}_j$.
    \item Second hidden layer: 3 neurons ($z^{(2)}_1 \dots z^{(2)}_3$) plus a bias unit ($z^{(2)}_0 = 1$). The weighted sum before activation is $h^{(2)}_k$.
    \item Output layer: 2 neurons ($z^{(3)}_1, z^{(3)}_2$). The weighted sum before activation is $h^{(3)}_l$.
\end{itemize}
All non-bias neurons use the sigmoid activation function: $z = \sigma(h) = \frac{1}{1+e^{-h}}$.

We denote:
\begin{itemize}
    \item $W^{(1)}$: weights from input to first hidden layer (size $4 \times 3$ because 4 hidden neurons each receive 2 inputs + bias).
    \item $W^{(2)}$: weights from first hidden to second hidden layer (size $3 \times 5$ because 3 hidden neurons each receive 4 inputs + bias).
    \item $W^{(3)}$: weights from second hidden to output layer (size $2 \times 4$ because 2 outputs each receive 3 inputs + bias).
\end{itemize}

\section{Forward Propagation}

\subsection{Coordinate Form (First Hidden Layer)}
For each neuron $j = 1,\dots,4$ in the first hidden layer:
\[
h^{(1)}_j = w^{(1)}_{j0} \cdot 1 + w^{(1)}_{j1} x_1 + w^{(1)}_{j2} x_2 = \sum_{i=0}^{2} w^{(1)}_{ji} x_i
\]
\[
z^{(1)}_j = \sigma\left(h^{(1)}_j\right) = \frac{1}{1+e^{-h^{(1)}_j}}
\]
where $x_0 = 1$ is the bias input.

\subsection{Coordinate Form (Second Hidden Layer)}
For each neuron $k = 1,\dots,3$ in the second hidden layer:
\[
h^{(2)}_k = w^{(2)}_{k0} \cdot 1 + \sum_{j=1}^{4} w^{(2)}_{kj} z^{(1)}_j = \sum_{j=0}^{4} w^{(2)}_{kj} z^{(1)}_j
\]
\[
z^{(2)}_k = \sigma\left(h^{(2)}_k\right)
\]
where $z^{(1)}_0 = 1$ is the bias activation from first hidden layer.

\subsection{Coordinate Form (Output Layer)}
For each output neuron $l = 1,2$:
\[
h^{(3)}_l = w^{(3)}_{l0} \cdot 1 + \sum_{k=1}^{3} w^{(3)}_{lk} z^{(2)}_k = \sum_{k=0}^{3} w^{(3)}_{lk} z^{(2)}_k
\]
\[
z^{(3)}_l = \sigma\left(h^{(3)}_l\right)
\]
where $z^{(2)}_0 = 1$ is the bias activation from second hidden layer.

\subsection{Matrix Representation}
Define the augmented input vectors (including bias):
\[
\mathbf{x} = \begin{bmatrix} 1 \\ x_1 \\ x_2 \end{bmatrix}, \quad
\mathbf{z}^{(1)} = \begin{bmatrix} 1 \\ z^{(1)}_1 \\ z^{(1)}_2 \\ z^{(1)}_3 \\ z^{(1)}_4 \end{bmatrix}, \quad
\mathbf{z}^{(2)} = \begin{bmatrix} 1 \\ z^{(2)}_1 \\ z^{(2)}_2 \\ z^{(2)}_3 \end{bmatrix}, \quad
\mathbf{z}^{(3)} = \begin{bmatrix} z^{(3)}_1 \\ z^{(3)}_2 \end{bmatrix}
\]

The weight matrices (including bias columns):
\[
W^{(1)} = \begin{bmatrix}
w^{(1)}_{10} & w^{(1)}_{11} & w^{(1)}_{12} \\
w^{(1)}_{20} & w^{(1)}_{21} & w^{(1)}_{22} \\
w^{(1)}_{30} & w^{(1)}_{31} & w^{(1)}_{32} \\
w^{(1)}_{40} & w^{(1)}_{41} & w^{(1)}_{42}
\end{bmatrix}_{4 \times 3}, \quad
W^{(2)} = \begin{bmatrix}
w^{(2)}_{10} & w^{(2)}_{11} & w^{(2)}_{12} & w^{(2)}_{13} & w^{(2)}_{14} \\
w^{(2)}_{20} & w^{(2)}_{21} & w^{(2)}_{22} & w^{(2)}_{23} & w^{(2)}_{24} \\
w^{(2)}_{30} & w^{(2)}_{31} & w^{(2)}_{32} & w^{(2)}_{33} & w^{(2)}_{34}
\end{bmatrix}_{3 \times 5}
\]
\[
W^{(3)} = \begin{bmatrix}
w^{(3)}_{10} & w^{(3)}_{11} & w^{(3)}_{12} & w^{(3)}_{13} \\
w^{(3)}_{20} & w^{(3)}_{21} & w^{(3)}_{22} & w^{(3)}_{23}
\end{bmatrix}_{2 \times 4}
\]

Forward propagation in matrix form:
\[
\mathbf{h}^{(1)} = W^{(1)} \mathbf{x}, \quad \mathbf{z}^{(1)} = \sigma(\mathbf{h}^{(1)}) \quad \text{(element-wise, ignoring bias row for now)}
\]
But careful: $\mathbf{z}^{(1)}$ in matrix form excludes the bias before the next layer. It's easier to keep bias separate.

Let $\mathbf{a}^{(1)} = [z^{(1)}_1, z^{(1)}_2, z^{(1)}_3, z^{(1)}_4]^T$ (no bias). Then:
\[
\mathbf{a}^{(1)} = \sigma\left( W^{(1)} \mathbf{x} \right)
\]
Then augment with bias: $\mathbf{z}^{(1)}_{\text{aug}} = [1; \mathbf{a}^{(1)}]$.

Similarly:
\[
\mathbf{a}^{(2)} = \sigma\left( W^{(2)} \mathbf{z}^{(1)}_{\text{aug}} \right), \quad \mathbf{z}^{(2)}_{\text{aug}} = [1; \mathbf{a}^{(2)}]
\]
\[
\mathbf{a}^{(3)} = \sigma\left( W^{(3)} \mathbf{z}^{(2)}_{\text{aug}} \right), \quad \mathbf{z}^{(3)} = \mathbf{a}^{(3)}
\]

\section{Backward Propagation (Training)}
We use the mean squared error loss for a single training sample $(\mathbf{x}, \mathbf{t})$ where $\mathbf{t} = [t_1, t_2]^T$ are the target outputs.
\[
E = \frac{1}{2} \sum_{l=1}^{2} (t_l - z^{(3)}_l)^2
\]

\subsection{Output Layer Gradients (Coordinate)}
Define the error signal for output neuron $l$:
\[
\delta^{(3)}_l = \frac{\partial E}{\partial h^{(3)}_l} = \frac{\partial E}{\partial z^{(3)}_l} \cdot \frac{\partial z^{(3)}_l}{\partial h^{(3)}_l} = \left( z^{(3)}_l - t_l \right) \cdot \sigma'(h^{(3)}_l)
\]
Since $\sigma'(h) = \sigma(h)(1-\sigma(h)) = z(1-z)$:
\[
\delta^{(3)}_l = (z^{(3)}_l - t_l) \cdot z^{(3)}_l (1 - z^{(3)}_l)
\]
Gradient for weights from second hidden layer to output:
\[
\frac{\partial E}{\partial w^{(3)}_{lk}} = \delta^{(3)}_l \cdot z^{(2)}_k, \quad \text{where } z^{(2)}_0 = 1
\]

\subsection{Second Hidden Layer Gradients (Coordinate)}
For neuron $k$ in the second hidden layer ($k=1,2,3$):
\[
\delta^{(2)}_k = \frac{\partial E}{\partial h^{(2)}_k} = \left( \sum_{l=1}^{2} \frac{\partial E}{\partial h^{(3)}_l} \cdot \frac{\partial h^{(3)}_l}{\partial z^{(2)}_k} \right) \cdot \frac{\partial z^{(2)}_k}{\partial h^{(2)}_k} = \left( \sum_{l=1}^{2} \delta^{(3)}_l w^{(3)}_{lk} \right) \cdot \sigma'(h^{(2)}_k)
\]
\[
\delta^{(2)}_k = \left( \sum_{l=1}^{2} \delta^{(3)}_l w^{(3)}_{lk} \right) \cdot z^{(2)}_k (1 - z^{(2)}_k)
\]
Gradients for weights from first hidden layer to second hidden layer:
\[
\frac{\partial E}{\partial w^{(2)}_{kj}} = \delta^{(2)}_k \cdot z^{(1)}_j, \quad \text{where } z^{(1)}_0 = 1
\]

\subsection{First Hidden Layer Gradients (Coordinate)}
For neuron $j$ in the first hidden layer ($j=1,2,3,4$):
\[
\delta^{(1)}_j = \frac{\partial E}{\partial h^{(1)}_j} = \left( \sum_{k=1}^{3} \frac{\partial E}{\partial h^{(2)}_k} \cdot \frac{\partial h^{(2)}_k}{\partial z^{(1)}_j} \right) \cdot \frac{\partial z^{(1)}_j}{\partial h^{(1)}_j} = \left( \sum_{k=1}^{3} \delta^{(2)}_k w^{(2)}_{kj} \right) \cdot \sigma'(h^{(1)}_j)
\]
\[
\delta^{(1)}_j = \left( \sum_{k=1}^{3} \delta^{(2)}_k w^{(2)}_{kj} \right) \cdot z^{(1)}_j (1 - z^{(1)}_j)
\]
Gradients for weights from input layer to first hidden layer:
\[
\frac{\partial E}{\partial w^{(1)}_{ji}} = \delta^{(1)}_j \cdot x_i, \quad \text{where } x_0 = 1
\]

\subsection{Matrix Representation of Backpropagation}
Define the error vectors (excluding bias neurons):
\[
\boldsymbol{\delta}^{(3)} = \begin{bmatrix} \delta^{(3)}_1 \\ \delta^{(3)}_2 \end{bmatrix}, \quad
\boldsymbol{\delta}^{(2)} = \begin{bmatrix} \delta^{(2)}_1 \\ \delta^{(2)}_2 \\ \delta^{(2)}_3 \end{bmatrix}, \quad
\boldsymbol{\delta}^{(1)} = \begin{bmatrix} \delta^{(1)}_1 \\ \delta^{(1)}_2 \\ \delta^{(1)}_3 \\ \delta^{(1)}_4 \end{bmatrix}
\]

Let $\odot$ denote element-wise multiplication. Then:
\[
\boldsymbol{\delta}^{(3)} = (\mathbf{z}^{(3)} - \mathbf{t}) \odot \mathbf{z}^{(3)} \odot (1 - \mathbf{z}^{(3)})
\]
\[
\boldsymbol{\delta}^{(2)} = \left( (W^{(3)}_{\text{no bias}})^T \boldsymbol{\delta}^{(3)} \right) \odot \mathbf{z}^{(2)} \odot (1 - \mathbf{z}^{(2)})
\]
where $W^{(3)}_{\text{no bias}}$ is $W^{(3)}$ without the bias column (i.e., columns corresponding to $k=1,2,3$ only), size $2 \times 3$.

\[
\boldsymbol{\delta}^{(1)} = \left( (W^{(2)}_{\text{no bias}})^T \boldsymbol{\delta}^{(2)} \right) \odot \mathbf{z}^{(1)} \odot (1 - \mathbf{z}^{(1)})
\]
where $W^{(2)}_{\text{no bias}}$ is $W^{(2)}$ without the bias column (i.e., columns corresponding to $j=1,2,3,4$ only), size $3 \times 4$.

The weight gradient matrices (including bias columns) are:
\[
\frac{\partial E}{\partial W^{(3)}} = \boldsymbol{\delta}^{(3)} (\mathbf{z}^{(2)}_{\text{aug}})^T, \quad
\frac{\partial E}{\partial W^{(2)}} = \boldsymbol{\delta}^{(2)} (\mathbf{z}^{(1)}_{\text{aug}})^T, \quad
\frac{\partial E}{\partial W^{(1)}} = \boldsymbol{\delta}^{(1)} \mathbf{x}^T
\]
where $\mathbf{z}^{(2)}_{\text{aug}} = [1, z^{(2)}_1, z^{(2)}_2, z^{(2)}_3]^T$ and $\mathbf{z}^{(1)}_{\text{aug}} = [1, z^{(1)}_1, z^{(1)}_2, z^{(1)}_3, z^{(1)}_4]^T$, and $\mathbf{x} = [1, x_1, x_2]^T$.
